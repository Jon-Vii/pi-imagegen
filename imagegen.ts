...
function render(){const visible=images.filter(passes);$('#count').textContent=visible.length+' image'+(visible.length===1?'':'s');$('#wall').innerHTML=visible.map(x=>'<button class="tile '+(selected?.imageId===x.imageId?'selected':'')+'" title="'+esc(x.prompt)+'" data-id="'+esc(x.imageId)+'"><img src="'+imgUrl(x)+'" loading="lazy"></button>').join('')||'<div class="empty"><h2>Start with a prompt.</h2><p>The canvas will fill with full, scaled-down images.</p></div>';document.querySelectorAll('.tile').forEach(c=>c.onclick=()=>select(c.dataset.id))}
